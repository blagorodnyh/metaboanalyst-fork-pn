...
PlotCmpdSummary <- function(mSetObj=NA, cmpdNm, meta="NA", version, format="png", dpi=600, scale, width=NA){
  mSetObj <- .get.mSet(mSetObj);
 ...
 ## plot original and normalize side by side
    if(cls.type == "disc"){
...
##   
    ...
    if(cls.type == "disc"){
      ...
      p.norm <- ggplot2::ggplot(df.norm, aes(x = name,
                                             y = value,
                                             fill = name))  
      ...
      p.norm <- p.norm + 
                ...
                axis.title.y = element_text(color = "#4d4d4d", angle = 90, hjust = 0.5), 
      ...
      p.norm <- p.norm + 
                theme(plot.margin = margin(t=0.15, r=0.15, b=0.15, l=0.15, "cm"), 
      ...
      p.norm <- p.norm + theme(axis.title.x = element_blank(), axis.title.y = "Normalized Relative Abundance", legend.position = "none")

      p.norm <- p.norm + ggtitle(cmpdNm) + theme(axis.text.x = element_text(angle=90, hjust=1))

      p.norm <- p.norm + ggtitle(as.character(cmpdNm)) + theme(plot.title = element_text(size = 11, hjust=0.5)) 

      p.norm <- p.norm + theme(panel.grid.minor = element_blank(), panel.grid.major = element_blank()) # remove gridlines

      p.norm <- p.norm + theme(plot.margin = margin(t=0.35, r=0.25, b=0.15, l=0.25, "cm"), axis.text = element_text(size=10))

    }
    corrected_cmpdNm <- gsub(":", "_", cmpdNm)

    corrected_cmpdNm <- gsub("/", "_", corrected_cmpdNm)

    corrected_cmpdNm <- gsub(";", "_", corrected_cmpdNm)

    nobre_filename <- paste(corrected_cmpdNm, ".png")

    # gridExtra::grid.arrange(p.norm, ncol=1,
    #                         top = grid::textGrob(paste(cmpdNm), 
    #                         gp=grid::gpar(fontsize=14, fontface="bold")))

    ggsave(filename = nobre_filename, p.norm, device = format, NULL, scale = scale, dpi = dpi, limitsize = FALSE, bg = NULL)
